...
CLASSES = (
    ...
    # ─── 以下為 §4 的 15 類**之外**的追加，需負責人核可 ───────────────
    ('calc_note', '計算式／法規檢討', 'annotation'),
)

# ★ 追加紀錄（append-only，id 永不重排）
#
# id 15 `calc_note` —— 2026-08-22 在 79 年案 p03 屋頂層平面圖上發現的缺口。
#
# ⚠ **規格 §4 寫的是 15 類，這是第 16 類，需要負責人核可。**
#
# 發現經過：判讀者讀到圖上的手寫算式
#     突一層：4.62×3.0 + 3.8×1.06 + 7.6×3.09 + … = 68.65 ＜ 565.11/8 = 70.64
# 並正確理解那是**屋突面積 ≤ 建築面積 1/8** 的建築技術規則檢討，
# 還主動指出「旁邊完全沒有尺寸線，這是註記型數字而非尺寸標註，
# 後續程式請勿嘗試配對 dim_line」。
#
# 但 15 類裡沒有能裝它的盒子，只好歸進 dim_text 並附警告 ——
# **理解是對的，是分類沒有位置。**
#
# 為什麼值得一個永久 id：台灣建照圖幾乎每張都有法規檢討
# （建蔽率、容積率、屋突面積、停車數量、法定空地），而且它們帶的是
# **導出的量化資訊**（本案由此得知總樓地板 565.11 ㎡、屋突 68.65 ㎡），
# 那是幾何算不出來、只能從圖上讀到的東西。
#
# 與 material_note 的分界：material_note 講**材料與構造**（1B磚、RC15、洗石子）；
# calc_note 講**數量與法規**（面積、比率、代入值、結論）。兩者都不帶尺寸線。

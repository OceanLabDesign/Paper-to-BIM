...
哪一道是依尺寸鏈（dimension chain）推的、哪一處圖上本來就對不起來。
...
一張 1979 年的曬圖掃進來：底色不均、對折處有摺痕（crease）白帶、
針筆（technical pen）線 0.4–0.6 mm 在 300 dpi 下只有 5–7 px、曬圖擴散（ink bleed）後兩條平行線會糊成一條粗帶。
在這種材料上：

- 尺寸標註（dimensioning）寫 `403`，傳統 OCR 可能給你一眼看得出是垃圾的 `4O3`；
  但視覺語言模型會給你**乾淨、合理、有自信、而且錯的 `408`**
- 一道 12 cm 的內牆，兩條線只隔 14 px，糊在一起就變一條線 —— 整面內牆從模型裡消失
- 二樓 X 向的尺寸鏈加總 12200，總尺寸標的卻是 12000

**最後那一項才是我們真正在處理的問題。** 那 200 的差額，是圖上本來就有矛盾（conflict），
...
**更多圖種（drawing type）。** 目前只處理 A 系列建築圖。結構圖（S 系列）有梁、有配筋，

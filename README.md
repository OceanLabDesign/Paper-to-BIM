...
**我們把民國 68 年（1979）的建照曬圖，重建成可用的 BIM 模型。**

> **現況：骨架階段。** 流程骨架與契約已經定下來，模組多數還是 stub，
...
一套把紙本建築圖轉成結構化 BIM 資料的流程。

不是 OCR 工具，也不是「上傳圖片、下載模型」的服務。我們做的是一條
**看得見自己哪裡不確定**的產線 —— 它會告訴你哪一道牆是量出來的、
哪一道是依尺寸鏈推的、哪一處圖上本來就對不起來。

輸入是掃描 PDF，輸出是 DXF（以及後續的 IFC），
中間每一步都落檔，每一個判斷都附得出證據。

## 為何而做

台灣有大量六〇、七〇年代的建照圖還躺在紙本或曬圖裡。
都更、危老重建、耐震評估、資產盤點 —— 每一項都要先有現況模型，
而現在的作法是找人照著舊圖重畫。慢、貴，而且**重畫的人會不自覺地把圖「修順」**。

我們想解決的其實不是「認字」，是**認錯了卻看起來很合理**這件事。
...
還是我們讀錯了？多數自動化系統會挑一邊、把差額均攤掉，
然後交出一個看起來很完整的模型 —— 而那個模型是錯的，且沒有人知道錯在哪。
...
迴圈次數、終止條件、退件重寫，我們全部寫死在 `planning/orchestrator.py` 的
...
讀數走三個**機制上獨立**的來源：OCR 的字形比對、視覺語言模型的語意理解、
...
我們支援本地模型不只是為了省錢：**掃描圖含個資與著作權，跑本地模型時圖不出機器。**
...
**座標必須追溯得到尺寸鏈或量測結果**（我們禁止目測座標）。

### 給人看的那一份

跑完之後，低信心與矛盾會匯成一份 Excel。我們一開始不做網頁校對介面 ——
Excel 先頂，人在上面改，改完的值優先於系統判讀。

### 檔名會告訴你可不可信

只要模型裡有任何一個值是從假設推來的（例如層高還沒從立面圖量到），
輸出檔名就會帶 `_draft`，而且該筆信心不超過 0.4。

**粗胚模型被誤當精確模型，是我們風險表上的中度風險。** 檔名是最後一道防線。
...
我們刻意讓**假中樞先於真中樞**：迴圈、驗收、執行、對照全部先用回固定計畫的
...
以下是方向，不是承諾 —— 我們連第 1 步都還沒走完。

**更多圖種。** 目前只處理 A 系列建築圖。結構圖（S 系列）有梁、有配筋，
是另一套詞彙；水電圖（M/E）又是一套。15 類偵測類別的設計刻意留了
append 的空間，但沒有插隊的空間。

**現況模型的下游。** 有了帶信心標記的現況模型，接得上耐震評估、
都更容積試算、資產盤點、建物履歷。關鍵在於**信心標記要跟著資料走** ——
我們把它寫進 IFC 的屬性集，不只寫在檔名。

**文資與歷史建築的數位典藏。** 這類案子的圖往往更舊、更爛，
但也最需要「不確定要留痕」這件事 —— 修復決策要有依據。

**不限台灣。** 任何有大量紙本圖說存量、又要做數位轉換的地方，
問題形狀是一樣的。校準參數要重測，架構不用改。

**架構本身可移植。** 「感知 → 判斷 → 執行，控制權在程式碼，矛盾不抹平」
這套骨架，處理的其實是更一般的問題：**從不可靠的來源重建結構化資料，
而且要能說明每一筆的把握程度**。舊圖只是我們選的第一個戰場。
...
| 套件 | 用途 | 授權 | 取得 |
|---|---|---|---|
| **PyMuPDF** | 讀 PDF 影像的放置矩陣 | ⚠ **AGPL-3.0 或 Artifex 商業授權（雙授權）** | `pip3 install pymupdf` |
| OpenCV | 二值化、霍夫轉換 | Apache-2.0 | `pip3 install opencv-python` |
...
>
> 我們選它是因為規格指定用它讀 PDF 影像 XObject 的放置矩陣（那是免拼接的關鍵）。
...
- **ArchiCAD 官方 API 建不了牆** —— 74 條命令裡沒有 `CreateWalls`，
  所以 Tapir 是硬相依，不是選配。它是社群專案，Graphisoft 沒有正式背書，
  裝進客戶端之前請確認貴方的合規要求
